...
future<service::topology_state_machine::topology_type> system_keyspace::load_topology_state() {
    ...
    for (auto& row : *rs) {
        ...
        if (row.has("topology_request")) {
            ...
            try {
                ret.requests.emplace(host_id, service::topology_request_from_string(val));
            } catch (...) {
        ...
        if ((nstate == service::node_state::replacing ||
            (ret.requests.contains(host_id) && ret.requests[host_id] == service::topology_request::replace)) && !replaced_id) {
            on_internal_error(slogger, fmt::format("replaced_id is missing for a node {}", host_id));
        }

        if (!tstate && t.size() != 0) {
            on_fatal_internal_error(slogger, "There cannot be tokens without the state");
        }
        ...
        if (map) {
            map->emplace(host_id, service::replica_state{nstate, std::move(datacenter), std::move(rack), std::move(release_version), replaced_id,
