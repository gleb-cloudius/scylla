    ...
    for (auto& row : *rs) {
        ...
        if (row.has("topology_request")) {
            ...
            try {
                ...
                slogger.error("Error while loading topology request '{}'. It will be ignored: {}", req, std::current_exception());
            }
            if (req == service::topology_request::replace) {
                if (!replaced_id) {
        ...
        if (nstate == service::node_state::rebuilding) {
            if (!rebuild_option) {
