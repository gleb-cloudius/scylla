...
raft_server_for_group raft_group0::create_server_for_group0(raft::group_id gid, raft::server_id my_id, service::storage_service& ss) {
    auto state_machine = std::make_unique<group0_state_machine>(_client, _mm, _qp.proxy(), ss);
...
future<> raft_group0::start_server_for_group0(raft::group_id group0_id, service::storage_service& ss) {
    ...
    co_await _raft_gr.start_server_for_group(create_server_for_group0(group0_id, my_id, ss));
    _group0.emplace<raft::group_id>(group0_id);
}

future<> raft_group0::join_group0(std::vector<gms::inet_address> seeds, bool as_voter, service::storage_service& ss) {
    ...
    if (group0_id) {
        ...
        co_return co_await start_server_for_group0(group0_id, ss);
    }
    ...
    while (true) {
        ...
        if (server == nullptr) {
            ...
            co_await start_server_for_group0(group0_id, ss);
...
future<> raft_group0::setup_group0(
        ...
        std::optional<replace_info> replace_info, service::storage_service& ss) {
    assert(this_shard_id() == 0);
    ...
    co_await join_group0(std::move(seeds), false /* non-voter */, ss);
...
future<> raft_group0::finish_setup_after_join(service::storage_service& ss) {
    if (joined_group0()) {
    ...
    co_await seastar::async([this, &ss] {
        _raft_support_listener = _feat.supports_raft_cluster_mgmt.when_enabled([this, &ss] {
            group0_log.info("finish_setup_after_join: SUPPORTS_RAFT feature enabled. Starting internal upgrade-to-raft procedure.");
            upgrade_to_group0(ss).get();
        });
    });
}
...
future<> raft_group0::upgrade_to_group0(service::storage_service& ss) {
    assert(this_shard_id() == 0);
    ...
    (void)[] (raft_group0& self, abort_source& as, group0_upgrade_state start_state, gate::holder pause_shutdown, service::storage_service& ss) -> future<> {
        auto warner = warn_if_upgrade_takes_too_long();
        try {
            co_await self.do_upgrade_to_group0(start_state, ss);
    ...
    }(std::ref(*this), std::ref(_abort_source), start_state, _shutdown_gate.hold(), ss);
}

// `start_state` is either `use_pre_raft_procedures` or `synchronize`.
future<> raft_group0::do_upgrade_to_group0(group0_upgrade_state start_state, service::storage_service& ss) {
    assert(this_shard_id() == 0);
    ...
    if (!joined_group0()) {
        ...
        co_await join_group0(co_await _sys_ks.load_peers(), true, ss);
    } else {
        upgrade_log.info(
