    ...
    if (raft_server) { // Raft is enabled. Check if we need to bootstrap ourself using raft
        ...
        if (leaving()) {
            if (_sys_ks.local().bootstrap_complete()) {
                ...
                throw std::runtime_error("Bootstrap failed. See earlier errors");
            }
        }
...
future<> storage_service::raft_decomission() {
    ...
    bool abort_when = false;

    auto f1 = _shutdown_request_promise.emplace().get_future().then([this, &abort_when] {
        // shutdown was signalled, abort topology even wait
       abort_when = true;
        _topology_state_machine.event.broadcast();
    });

    auto f2 = _topology_state_machine.event.wait([this, &raft_server, &abort_when] {
        if (abort_when) {
