...
future<> storage_service::topology_change_coordinator_fiber(raft::server& raft, sharded<db::system_distributed_keyspace>& sys_dist_ks, abort_source& as) {
    ...
    while (!as.abort_requested()) {
        try {
            ...
            auto it = topo.transition_nodes.end();

            if (topo.transition_nodes.size() != 0) {
                // If there is a node that is the middle of topology operation continue with it
                it = topo.transition_nodes.begin();
            } else if (topo.new_nodes.size() != 0) {
                // Otherwise check if there is a new node that wants to be joined
                it = topo.new_nodes.begin();
            } else {
                // If there is no new node, search if there is a request on existing node
                it = boost::find_if(topo.normal_nodes, [] (auto& n) { return bool(n.second.topology_request); });
            }

            if (it == topo.normal_nodes.end()) {
            ...
            switch (rs.state) {
                ...
                case node_state::none: {
                    ...
                    // if the state is normal there have to be either 'leave' or 'remove' request
                    assert(rs.topology_request);
    ...
    if (raft_server) { // Raft is enabled. Check if we need to bootstrap ourself using raft
       ...
       // Wait until we enter one of the final states
        while (!_topology_change_sm._topology.normal_nodes.contains(raft_server->id()) &&
               !_topology_change_sm._topology.left_nodes.contains(raft_server->id())) {
            co_await _topology_change_sm.event.wait();
        }

        if (_topology_change_sm._topology.left_nodes.contains(raft_server->id())) {
