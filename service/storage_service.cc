...
future<> storage_service::topology_change_coordinator_fiber(raft::server& raft, sharded<db::system_distributed_keyspace>& sys_dist_ks, abort_source& as) {
    ...
    auto handle_node_transition = [this, &handle_ring_transition, &update_replica_state, &exec_direct_command] (topology_snapshot&& node) -> future<> {
        ...
        switch (node.rs.state) {
            ...
            case node_state::replacing:
                ...
                break;
            case node_state::rebuilding: {
                co_await exec_direct_command(node.id, raft_topology_cmd{raft_topology_cmd::command::stream_ranges});
                db::system_keyspace::topology_mutation_builder builder(node.guard.write_timestamp(), node.id);
    ...
    auto get_node_to_work_on = [this] (abort_source& as) -> future<std::optional<topology_snapshot>> {
        ...
        std::optional<request_param> req_param;
        auto rit = topo.req_param.find(it->first);
        if (rit != topo.req_param.end()) {
            req_param = topo.req_param[it->first];
        }
        co_return topology_snapshot{std::move(guard), topo, it->first, it->second, std::move(req), std::move(req_param)};
    };
...
future<> storage_service::raft_rebuild(sstring source_dc) {
    ...
    while (true) {
        ...
        builder.set("rebuild_option", source_dc);
        topology_change change{{builder.build()}};
