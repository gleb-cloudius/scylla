...
future<> storage_service::topology_state_load() {
    ...
    for (const auto& [id, rs]: _topology_state_machine._topology.left_nodes) {
        assert(!rs.ring);
    ...
    co_await mutate_token_metadata(seastar::coroutine::lambda([this, &id2ip, &am] (mutable_token_metadata_ptr tmptr) -> future<> {
        ...
        for (const auto& [id, rs]: _topology_state_machine._topology.normal_nodes) {
            assert (rs.ring.value().state == ring_state::ring_state::replication_state::owner);
        ...
        for (const auto& [id, rs]: _topology_state_machine._topology.transition_nodes) {
            ...
            case node_state::replacing: {
                auto replaced_id = rs.replaced_id.value();
...
future<> storage_service::topology_change_coordinator_fiber(raft::server& raft, sharded<db::system_distributed_keyspace>& sys_dist_ks, abort_source& as) {
    ...
    auto exec_global_command = [this, &exec_direct_command] (topology_state_machine::topology_type& topo, const raft_topology_cmd& cmd, bool include_local, raft::server_id exclude_node) -> future<bool> {
        ...
        auto f = co_await coroutine::as_future(seastar::parallel_for_each(std::move(nodes), [&cmd, &exec_direct_command] (raft::server_id id) {
            return exec_direct_command(id, cmd);
        }));
        if (f.failed()) {
    ...
    auto handle_ring_transition = [&] (group0_guard& guard, topology_state_machine::topology_type& topo, raft::server_id id, const replica_state& rs) -> future<> {
        raft::server_id replaced_node;

        if (rs.state == node_state::replacing) {
            replaced_node = rs.replaced_id.value();
        }

        switch (rs.ring.value().state) {
            case ring_state::ring_state::replication_state::write_only: {
                ...
                // Streaming completed. We can now move tokens state to ring_state::ring_state::replication_state::read_write
                db::system_keyspace::topology_mutation_builder builder(guard.write_timestamp(), id);
                builder.set("replication_state", ring_state::replication_state::read_write);
                co_await update_replica_state(guard, {builder.build()}, fmt::format("{}: streaming completed", rs.state));
            }
                break;
            case ring_state::replication_state::read_write:
                ...
                case node_state::bootstrapping: {
                    db::system_keyspace::topology_mutation_builder builder(guard.write_timestamp(), id);
                    builder.set("replication_state", ring_state::replication_state::owner)
                           .set("node_state", node_state::normal);
                    co_await update_replica_state(guard, {builder.build()}, "bootstrap: read fence completed");
                    }
                    break;
                case node_state::decommissioning:
                case node_state::removing: {
                    ...
                    co_await update_replica_state(guard, {builder.build()}, fmt::format("{}: read fence completed", rs.state));
                    }
                    break;
                case node_state::replacing: {
                    db::system_keyspace::topology_mutation_builder builder1(guard.write_timestamp(), id);
                    // Move new node to 'normal'
                    builder1.set("replication_state", ring_state::replication_state::owner)
                            .set("node_state", node_state::normal);
                    ...
                    builder2.del("replication_state")
                            .del("tokens")
                            .set("node_state", node_state::left);
                    co_await update_replica_state(guard, {builder1.build(), builder2.build()}, "replace: read fence completed");
                    }
                    break;
                default:
                    on_fatal_internal_error(slogger, format("Ring state on node {} is read_write while the node is in state {}", id, rs.state));
                }
                // Reads are fenced. We can now move tokens state to ring_state::replication_state::owner and node to normal
                break;
            case ring_state::replication_state::owner:
                // should not get here
                on_fatal_internal_error(slogger, format("Tried to handle ring state transition on node {} while in 'owner' state", id));
                break;
        }
    };
    ...
    while (!as.abort_requested()) {
        try {
            ...
            if (topo.transition_nodes.size() != 0) {
                ...
                it = boost::find_if(topo.normal_nodes, [] (auto& n) { return bool(n.second.request); });
            }
            ...
            switch (rs.state) {
                ...
                case node_state::none: {
                    ...
                    switch (rs.request.value()) {
                        ...
                        builder.set("node_state", node_state::bootstrapping)
                               ...
                               .set("replication_state", ring_state::replication_state::write_only);
                        co_await update_replica_state(guard, {builder.build()}, "bootstrap: assign tokens");
                        break;
                        }
                    case topology_request::leave:
                        ...
                        builder.set("node_state", node_state::decommissioning)
                               .del("topology_request")
                               .set("replication_state", ring_state::replication_state::write_only);
                        co_await update_replica_state(guard, {builder.build()}, "start decommission");
                        break;
                    case topology_request::remove:
                        ...
                        builder.set("node_state", node_state::removing)
                               .del("topology_request")
                               .set("replication_state", ring_state::replication_state::write_only);
                        co_await update_replica_state(guard, {builder.build()}, "start removenode");
                        break;
                    case topology_request::replace: {
                        ...
                        builder.set("node_state", node_state::replacing)
                               ...
                               .set("replication_state", ring_state::replication_state::write_only);
                        co_await update_replica_state(guard, {builder.build()}, "start replace");
                        break;
                        }
                    }
                    break;
                ...
                case node_state::left:
                    // Should not get here
                    on_fatal_internal_error(slogger, format("Topology coordinator is called for node {} in state 'left'", id));
                    break;
            }
...
future<> storage_service::raft_replace(raft::server& raft_server, raft::server_id replaced_id, gms::inet_address replaced_ip) {
    ...
    while (!_topology_state_machine._topology.contains(raft_server.id())) {
        ...
        if (!_topology_state_machine._topology.normal_nodes.contains(replaced_id)) {
            throw std::runtime_error(fmt::format("Cannot replace node {}/{} because it is not in the 'normal' state", replaced_ip, replaced_id));
        }

        auto& rs = _topology_state_machine._topology.normal_nodes[replaced_id];
...
future<> storage_service::raft_removenode(locator::host_id host_id) {
    ...
    while (true) {
        ...
        if (rs.state == node_state::left) {
            throw std::runtime_error(format("raft topology removenode: node {} already left the cluster", id));
        }
        ...
        try {
            ...
            switch (cmd.cmd) {
                ...
                case raft_topology_cmd::command::stream_ranges: {
                    ...
                    if (!rs.ring || (rs.ring->state != ring_state::replication_state::write_only && rs.state != node_state::normal)) {
                    ...
                    case node_state::replacing: {
                        ...
                        if (rs.state == node_state::bootstrapping) {
                            if (_topology_state_machine._topology.normal_nodes.size()) {
                    ...
                    break;
                    case node_state::normal: {
                        // If asked to stream a node in normal state it mneans that remove operation is running
