...
class topology_coordinator : public endpoint_lifecycle_subscriber {
    ...
    std::unordered_set<raft::server_id> get_dead_node(bool& cleanup_running, bool& cleanup_needed) {
        ...
        for (auto& n : _topo_sm._topology.normal_nodes) {
            if (!_gossiper.is_alive(id2ip(locator::host_id(n.first.uuid())))) {
    ...
    std::variant<group0_guard, cancel_requests, start_cleanup, node_to_work_on> get_node_to_work_on_opt(group0_guard guard) {
        auto& topo = _topo_sm._topology;
        ...
        auto get_req_param = [&topo] (raft::server_id id) {
            std::optional<request_param> req_param;
            auto rit = topo.req_param.find(id);
        ...
        bool cleanup_needed;
        auto dead_nodes = get_dead_node(cleanup_running, cleanup_needed);
        ...
        for (auto& req : topo.requests) {
            auto enough_live_nodes = [&] {
                auto exclude_nodes = get_excluded_nodes(req.first, req.second, get_req_param(req.first));
        ...
        return node_to_work_on(std::move(guard), &topo, id, &topo.find(id)->second, req, get_req_param(id));
    };
    ...
    std::pair<bool, group0_guard> should_preempt_balancing(group0_guard guard) {
        auto work = get_node_to_work_on_opt(std::move(guard));
    ...
    while (!_as.abort_requested()) {
        ...
        try {
            co_await utils::get_local_injector().inject_with_handler("topology_coordinator_pause_before_processing_backlog",
                [] (auto& handler) { return handler.wait_for_message(db::timeout_clock::now() + std::chrono::seconds(10)); });
            auto guard = co_await start_operation();
