...
class topology_coordinator {
    ...
    std::unordered_set<raft::server_id> get_excluded_nodes(node_to_work_on& node) {
...
future<> topology_coordinator::rollback_current_topology_op(group0_guard&& guard) {
    ...
    switch (node.rs->state) {
        ...
        case node_state::removing:
            ...
            [[fallthrough]];
        case node_state::decommissioning:
            // to rollback decommission or remove just move a node that wwe tried to remove back to normal state
    ...
    if (raft_server) { // Raft is enabled. Check if we need to bootstrap ourself using raft
        ...
        if (leaving()) {
            if (_sys_ks.local().bootstrap_complete()) {
                ...
                throw std::runtime_error("Bootstrap failed. See early errors");
            }
        }
...
future<> storage_service::raft_decomission() {
    ...
    bool left = false;
    co_await _topology_state_machine.event.when([this, &raft_server, &left] {
        // Wait for this node to move to state left or left_token_ring which means that decomission completed
        // or wait for decomission request to be removed, but node stay as normal which means decomission failed
        auto it = _topology_state_machine._topology.find(raft_server.id());
        ...
        if (it->second.state == node_state::normal) {
            ...
            if (rit == _topology_state_machine._topology.requests.end() || rit->second != topology_request::leave) {
                return true; // node is normal, but leave request is gone. It means decomission failed
            }
        }
        return false;
    });
    ...
    if (left) {
        ...
        const auto err = "Decommission failed. See early errors";
        slogger.error(err);
        throw std::runtime_error(err);
    }
}
...
future<> storage_service::raft_removenode(locator::host_id host_id, std::list<locator::host_id_or_endpoint> ignore_nodes_params) {
    ...
    co_await _topology_state_machine.event.when([this, id, &left] {
        // Wait for this node to move to state left which means that decomission completed
        // or wait for removenode request to be removed, but node stay as normal which means decomission failed
        auto it = _topology_state_machine._topology.find(id);
        if (!it) {
            left = true;
            return true; // node either left or on the way
        ...
        if (it->second.state == node_state::normal) {
            ...
            if (rit == _topology_state_machine._topology.requests.end() || rit->second != topology_request::remove) {
                return true; // node is normal, but remove request is gone. It means decomission failed
            }
        }
        return false;
    });
    ...
    if (left) {
        ...
        const auto err = "Removenode failed. See early errors";
        slogger.error(err);
        throw std::runtime_error(err);
    }
}
