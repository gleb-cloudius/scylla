...
class topology_coordinator {
        ...
        if (wait_for_cleanup) {
            ...
            for (auto& [id, rs] : _topo_sm._topology.normal_nodes) {
                if (!exclude_nodes.contains(id) && rs.cleanup_needed) {
                    ...
                    // start working new topology operation. Return only the guard to indicate it.
    ...
    future<> handle_node_transition(node_to_work_on&& node) {
        ...
        switch (node.rs->state) {
            ...
            case node_state::rollback_to_normal: {
                ...
                auto str = fmt::format("complete rollback of {} to state normal and setting cleanup flag", node.id);
    ...
    std::vector<canonical_mutation> mark_all_nodes_as_cleanup_needed(api::timestamp_type timestamp) {
        auto& topo = _topo_sm._topology;
        std::vector<canonical_mutation> muts;
        muts.reserve(topo.normal_nodes.size());
        for (auto& [id, rs] : topo.normal_nodes) {
            topology_mutation_builder builder(timestamp);
            builder.with_node(id).set("cleanup_needed", true);
            muts.emplace_back(builder.build());
