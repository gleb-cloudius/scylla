...
future<> storage_service::raft_decomission() {
    ...
    co_await _topology_state_machine.event.when([this, &raft_server, &left] {
        // Wait for this node to move to state left or left_token_ring which means that decommission completed
        // or wait for decommission request to be removed, but node stay as normal which means decommission failed
        auto it = _topology_state_machine._topology.find(raft_server.id());
