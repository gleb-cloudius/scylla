...
cql3::statements::alter_keyspace_statement::execute(query_processor& qp, service::query_state& state, const query_options& options, service::group0_guard* guard) const {
    std::optional<sstring> warning = check_restricted_replication_strategy(qp, keyspace(), *_attrs);
    return schema_altering_statement::execute(qp, state, options, guard).then([warning = std::move(warning)] (::shared_ptr<messages::result_message> msg) {
        if (warning) {
