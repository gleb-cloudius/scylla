...
strongly_consistent_modification_statement::execute(query_processor& qp, service::query_state& qs, const query_options& options, service::group0_guard* guard) const {
    return execute_without_checking_exception_message(qp, qs, options, guard)
...
future<::shared_ptr<cql_transport::messages::result_message>>
strongly_consistent_modification_statement::execute_without_checking_exception_message(query_processor& qp, service::query_state& qs, const query_options& options, service::group0_guard* guard) const {
    if (this_shard_id() != 0) {
