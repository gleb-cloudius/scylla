...
private:
    ...
    std::unordered_set<inet_address> _dirty_endpoints;
...
public:
    explicit vnode_effective_replication_map(replication_strategy_ptr rs, token_metadata_ptr tmptr, replication_map replication_map,
            ring_mapping pending_endpoints, ring_mapping read_endpoints, std::unordered_set<inet_address> dirty_endpoints, size_t replication_factor) noexcept
    ...
    struct cloned_data {
        ...
        std::unordered_set<inet_address> dirty_endpoints;
    };
    ...
    // Returns a set of dirty endpoint. Will be empty if there is no topology change.
    // During topology can be empty as well (for instance for everywhere strategy)
    const std::unordered_set<inet_address>& get_dirty_endpoints() const {
        return _dirty_endpoints;
    }

    std::unordered_set<inet_address> get_all_pending_nodes() const;
...
inline mutable_vnode_erm_ptr make_effective_replication_map(replication_strategy_ptr rs, token_metadata_ptr tmptr, replication_map replication_map, ring_mapping pending_endpoints,
    ring_mapping read_endpoints, std::unordered_set<inet_address> dirty_endpoints, size_t replication_factor) {
